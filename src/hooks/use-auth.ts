...
interface AuthContextType {
  user: User | null;
  loading: boolean;
  login: (email: string, pass: string, callbacks: { onSuccess: () => void; onError: (msg: string) => void; }) => Promise<void>;
  logout: () => void;
}
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const login = async (email: string, pass: string, callbacks: { onSuccess: () => void; onError: (msg: string) => void; }) => {
    try {
